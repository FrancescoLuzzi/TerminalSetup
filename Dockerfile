...
RUN apt update && \
    apt upgrade -y && \
    mkdir ~/.config && \
    apt -y install sudo procps file git bash-completion curl wget tree zip build-essential libssl-dev libffi-dev apt-utils iputils-ping locales ripgrep fzf
